...
export function LandingPage({ onAnalyze, onCompare, aiEnabled, onOpenSettings, onOpenProfile, onOpenPricing, onOpenMetiers }: Props) {
  ...
  // Échange de place Lumi ↔ Luminator au clic (toggle).
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative mx-auto max-w-4xl px-6 pt-8 pb-16 text-center md:pt-12">
        {/* Scène : Lumi et Luminator. L'un est au premier plan (net, centré),
            l'autre en retrait (flou, à droite). Cliquer sur celui en retrait
            les fait échanger de place. */}
        <div className="animate-fade-in relative mx-auto h-60 w-full max-w-sm md:h-72">
          {owns ? (
            <Avatar state="idle" className="h-full w-full" />
          ) : (
            <>
              {/* Lumi : devant par défaut, recule quand on échange */}
              <div
                className={`absolute inset-0 origin-center transition-all duration-700 ease-out ${
                  swapped ? 'z-10 translate-x-[36%] -translate-y-[8%] scale-[0.55] opacity-40 blur-[3px]' : 'z-20'
                }`}
              >
                <Avatar state="idle" glasses={false} className={`h-full w-full ${swapped ? 'pointer-events-none' : ''}`} />
              </div>

              {/* Luminator : en retrait par défaut, avance quand on échange */}
              <div
                className={`absolute inset-0 origin-center transition-all duration-700 ease-out ${
                  swapped ? 'z-20' : 'z-10 translate-x-[36%] -translate-y-[8%] scale-[0.55] opacity-40 blur-[3px]'
                }`}
              >
                <Avatar state="idle" glasses className={`h-full w-full ${swapped ? '' : 'pointer-events-none'}`} />
              </div>

              {/* Zone cliquable sur le personnage en retrait (à droite) → échange */}
              <button
                onClick={() => setSwapped((s) => !s)}
                aria-label="Échanger Lumi et Luminator"
                title="Échanger 🔄"
        ...
        {/* Bulle : message du personnage qui est devant */}
            ...
            {swapped && !owns ? (
              ...
              <>
                ...
                <span className="block text-xs text-ink-400">
                  {owns ? '(tapote-moi sur la tête 👆)' : '(tapote-moi 👆 — clique sur Luminator à droite pour échanger 🔄)'}
                </span>
              </>
            )}
          </div>
