...
export async function sendMagicLink(to: string, link: string): Promise<void> {
  ...
  <div style="font-family:system-ui,Arial,sans-serif;max-width:480px;margin:auto">
    <h2 style="color:#1c2033">Ta connexion à Lumi</h2>
    <p style="color:#434a6a">Clique sur le bouton ci-dessous pour te connecter. Ce lien expire dans 15 minutes.</p>
    <p style="margin:24px 0">
      <a href="${link}" style="background:#4f46e5;color:#fff;text-decoration:none;padding:12px 20px;border-radius:12px;font-weight:600">Se connecter à Lumi</a>
    </p>
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from, to, subject: 'Ton lien de connexion Lumi', html }),
  })
